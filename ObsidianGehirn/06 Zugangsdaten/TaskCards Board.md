...
> [!failure] Letzte Workshop-Nacht bestätigt (Nacht 11.→12.09.2026) – gleiche Ursache, jetzt mit klarerer Diagnose
> Auch mit einem echten headless Chromium via Playwright (nicht nur `curl`) identischer Befund: `page.goto` scheitert mit `net::ERR_TUNNEL_CONNECTION_FAILED`. Der Proxy-Status-Endpoint zeigt den `403`/`connect_rejected` auf `itech-bs14.taskcards.app:443` **im selben Batch wie zwei identische Ablehnungen für `www.google.com:443`** – die Sandbox-Policy blockt also praktisch jeden externen Host außerhalb einer Allowlist, nicht spezifisch TaskCards. Zugleich bestätigt Issue [#100](https://github.com/47Felix/RasberryPI-Team-13/issues/100), dass der dokumentierte "Fix" ein **lokaler Cronjob (`board-sync.sh`) auf der Team-VM** ist – ein separater Ausführungskontext, der diese Cloud-Sandbox-Automation nicht betrifft. Beide Mechanismen dürften also unabhängig voneinander laufen (oder eben nicht laufen); das wurde nie im Team gegengeprüft. Da mit dem 12.09. der letzte Tag des DTEW-Zeitraums endet, sollte vor dem endgültigen Abschluss geklärt werden, ob `board-sync.sh` tatsächlich zuverlässig lief, und ob die nächtliche Cloud-Automation danach überhaupt noch gebraucht wird (siehe [[DTEW 0912 - Freier Tag, Bremen-Exkursion und Workshop-Abschluss]]).
