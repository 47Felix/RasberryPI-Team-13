...
### Automatischer Kontext-Reset nach 12 Nachrichten (seit 01.09.2026, PR [#67](https://github.com/47Felix/RasberryPI-Team-13/pull/67); Schwelle am 17.09.2026 von 25 auf 12 gesenkt, PR [#198](https://github.com/47Felix/RasberryPI-Team-13/pull/198))

Sessions liefen vorher unbegrenzt mit `--resume` auf derselben `session_id` weiter, wodurch die pro Nachricht mitgeschickten Tokens (und damit der Verbrauch) stetig wuchsen. Jetzt zählt der Bot pro User die Nachrichten mit (`message_count` in `users.json`) und verwirft die `session_id` automatisch nach `AUTO_RESET_AFTER_MESSAGES = 12` Nachrichten (Konstante in `bot.py`, ursprünglich 25 – `--resume` hängt bei jeder Nachricht die komplette bisherige Historie an, wodurch lange Threads jede einzelne Folgenachricht teurer machen, daher die Schwelle weiter gesenkt) – die nächste Nachricht startet dann eine frische Session, der Bot meldet das im Kanal. Zusätzlich gibt es den manuellen Befehl `!clear`, der Session und Zähler sofort zurücksetzt. Registrierung, Config-Dir und Worktree bleiben davon unberührt – niemand muss sich neu per `!register` anmelden.
...
- **Cron:** täglich `19:00 UTC` (= 21:00 Sommerzeit/CEST) – `crontab -l` auf der VM zeigt den aktiven Eintrag. War ursprünglich abends (19:30 UTC), dann morgens (04:45 UTC), dann nachts 2 Uhr Ortszeit (01.09.2026), auf Felix' Wunsch am 03.09.2026 auf abends 21 Uhr Ortszeit umgestellt.
  > [!warning] CRON_TZ funktioniert NICHT auf dieser VM
  > Erster Versuch war `CRON_TZ=Europe/Berlin` + `45 6 * * *`, damit sich die Zeit automatisch an Sommer-/Winterzeit anpasst. Per Test am 27.08.2026 verifiziert: dieser Cron (`cron 3.0pl1-184ubuntu2`, Ubuntu) **ignoriert `CRON_TZ` und nimmt immer die Systemzeit** (VM läuft in UTC, siehe `timedatectl`). Deshalb fest UTC-Uhrzeit ohne `CRON_TZ`.
  > **Konsequenz:** Wenn Deutschland auf Winterzeit (CET, UTC+1) wechselt (letzter Sonntag im Oktober), muss die Crontab manuell von `0 19` auf `0 20` geändert werden, sonst läuft der Sync ab dann um 20:00 statt 21:00 Uhr Ortszeit – siehe [[Offene Punkte]].
- **Account/Config-Dir:** eigener, von den User-Sessions getrennter Ordner `~/sessions/vault-sync/` (Credentials von `amogus_911` übernommen, mit dessen Zustimmung – kein separates `claude login` nötig). **Seit 28.08.2026:** Das Skript kopiert die Credentials vor **jedem** Lauf frisch von der aktiven `amogus_911`-Session, statt sich auf eine einmalige statische Kopie zu verlassen (siehe Incident unten). **Seit 03.09.2026:** Die Kopie wird zusätzlich validiert (nicht-leerer Access-Token, gesetztes Ablaufdatum) und bei Bedarf bis zu 5x mit 3s Abstand wiederholt, da ein einzelner `cp` mitten in einem Token-Refresh der Quelldatei kurzzeitig leere Werte liefern kann (siehe Incident unten).
...
### Incident: Race Condition beim Credential-Kopieren (03.09.2026)
Root Cause: Der seit dem Incident vom 27./28.08. eingebaute frische `cp` der `amogus_911`-Credentials vor jedem Lauf traf am 03.09.2026 um 00:00 Uhr UTC genau den Moment eines laufenden Token-Refreshs dieser Session. Die kopierte `.credentials.json` hatte dadurch kurzzeitig `accessToken: ""` und `expiresAt: 0`. Der anschließende `claude -p ...`-Aufruf schlug dadurch sofort (< 1s) mit Auth-Fehler und Exit-Code 1 fehl – der ERR-Trap griff zwar und meldete den Abbruch korrekt in Discord, aber ohne aussagekräftigen Fehlertext im Log (Zeile 108, Exit-Code 1), da die eigentliche Ursache – ein leeres Token statt eines echten API-Fehlers – nirgends explizit sichtbar war. Gefixt: `vault-sync.sh` validiert die kopierte Datei jetzt (nicht-leerer `accessToken`, `expiresAt` gesetzt) und wiederholt den Kopiervorgang bei Bedarf bis zu 5x mit 3s Abstand, bevor es weitermacht.
