...
> [!info] Stand
> 26.08.2026 – Grundgerüst gebaut und einmal erfolgreich getestet (erste Nachricht beantwortet, Test-Branch/Commit erzeugt). Noch nicht alle Teammitglieder registriert, systemd-Dauerbetrieb ggf. noch einzurichten (siehe [[Offene Punkte]]).
...
- **Bot-Ordner auf der VM:** `~/discord-claude-bot/` mit `bot.py`, `requirements.txt`, `.env` (Bot-Token, Channel-ID, Repo-Pfad – nie committen), `users.json` (Mapping Discord-User-ID → Config-Verzeichnis + Session-ID)
...
Läuft aktuell mit `--dangerously-skip-permissions`, weil es über Discord keine Möglichkeit gibt, eine interaktive Freigabe-Rückfrage zu beantworten. Heißt: Claude führt Aktionen (Dateiänderungen, Git-Befehle, Shell-Kommandos) im Namen der jeweiligen Person **sofort ohne Rückfrage** aus.

> [!warning] Bewusster Trade-off
> Für ein privates Team-Repo vertretbar, aber es bedeutet: ein unbedachter Prompt im Discord-Thread kann direkt Dateien ändern, Branches erzeugen o.ä. Alternative für mehr Kontrolle wäre eine feinere Tool-Allowlist statt komplettem Skip – siehe [[Offene Punkte]].
...
## Offene Punkte
Siehe [[Offene Punkte]] – u.a. weitere Teammitglieder registrieren, Discord-Bot-Token rotieren, Entscheidung Session-pro-Thread vs. Session-pro-User.
