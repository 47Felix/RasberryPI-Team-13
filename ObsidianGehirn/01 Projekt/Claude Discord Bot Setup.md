...
- **Cron:** täglich `04:45 UTC` (= 06:45 Sommerzeit/CEST) – `crontab -l` auf der VM zeigt den aktiven Eintrag. War ursprünglich abends (19:30 UTC), auf Wunsch auf morgens umgestellt.
  > [!warning] CRON_TZ funktioniert NICHT auf dieser VM
  > Erster Versuch war `CRON_TZ=Europe/Berlin` + `45 6 * * *`, damit sich die Zeit automatisch an Sommer-/Winterzeit anpasst. Per Test am 27.08.2026 verifiziert: dieser Cron (`cron 3.0pl1-184ubuntu2`, Ubuntu) **ignoriert `CRON_TZ` und nimmt immer die Systemzeit** (VM läuft in UTC, siehe `timedatectl`). Ein testweise gesetzter Job feuerte nicht zur berechneten Berlin-Zeit, sondern erst 2h später zur UTC-Literalzeit. Deshalb jetzt fest `45 4 * * *` (UTC) ohne `CRON_TZ`.
  > **Konsequenz:** Wenn Deutschland auf Winterzeit (CET, UTC+1) wechselt (letzter Sonntag im Oktober), muss die Crontab manuell von `45 4` auf `45 5` geändert werden, sonst läuft der Sync ab dann um 05:45 statt 06:45 Uhr Ortszeit – siehe [[Offene Punkte]].
