...
> [!danger] Nachtrag 12.09. abends: Automation lief über das Workshop-Ende hinaus – Trigger sollte deaktiviert werden
> Diese Notiz wurde in der Nacht auf Samstag 12.09. für **Samstag 12.09.** geschrieben (letzter Workshop-Tag). Die zugehörige nächtliche Automation ist aber weiterhin scharf geschaltet und feuerte am **Samstagabend, 12.09.** erneut – diesmal zur Vorbereitung von **Sonntag, 13.09.** Laut [[DTEW Hamburg - Übersicht]] endet der Zeitplan jedoch mit Samstag 12.09. ("letzter Tag in Deutschland zur freien Verfügung"); für Sonntag 13.09. existiert **kein** Übersichts-Eintrag und damit keine Aufgabe, die diese Automation lösen könnte – der Workshop ist nach heute planmäßig vorbei.
> - **Board-Zugriff erneut getestet** (Playwright-Voraussetzung: `curl` gegen `itech-bs14.taskcards.app:443` direkt über den Sandbox-Proxy): weiterhin `net::ERR_TUNNEL_CONNECTION_FAILED` / `connect_rejected (403)` – identisch zum seit 03.09. dokumentierten Befund. Da es ohnehin keine Sonntags-Aufgabe zu posten gibt, wurde kein weiterer Playwright-Versuch unternommen.
> - **Trigger-Übersicht geprüft** (`list_triggers`): Die drei anderen nächtlichen DTEW-Trigger ("DTEW Morning Ping", "DTEW Nightly Prototype Dev", "DTEW Nightly Board Sync") sind bereits `enabled: false` – jemand/eine frühere Session hat sie offenbar schon abgeschaltet. Nur **"DTEW Feed-Diversity-Prototyp: naechtlicher Ausbau"** (`trig_018KNvau7tFqPjwsTWfrwDdB`, Cron `0 20 * * *`, also täglich 20:00 UTC) läuft noch – das ist vermutlich der Trigger, der auch diese Nacht wieder feuert.
> - **Empfehlung:** Da der Workshop nach dem 12.09. planmäßig beendet ist, sollte das Team diesen letzten verbliebenen Trigger jetzt explizit deaktivieren/löschen (`trig_018KNvau7tFqPjwsTWfrwDdB`), statt ihn nachts weiter ins Leere laufen zu lassen. Es wurde heute Nacht **keine neue Notiz für den 13.09. angelegt und nichts an die Boards gepostet**, da es dafür keine reale Aufgabe gibt – ein Platzhalter-Eintrag hätte einen nicht existierenden Workshop-Tag vorgetäuscht.
...
- [ ] **Trigger `trig_018KNvau7tFqPjwsTWfrwDdB` ("DTEW Feed-Diversity-Prototyp: naechtlicher Ausbau") deaktivieren** – letzter noch aktiver nächtlicher DTEW-Trigger, feuerte am Abend des 12.09. bereits für einen nicht existierenden 13.09. (s. Nachtrag oben); die drei anderen DTEW-Nacht-Trigger sind bereits deaktiviert
