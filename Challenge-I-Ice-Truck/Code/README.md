...
- `pi-backend/` - Track C, E, F (Pi-Seite): `hardware.py` (I2C real+mock, zwei Adressen), `calibration.py` (Rohwert → Grad Celsius, kalibriert), `rules.py` (Regellogik), `db.py` (SQLite), `app.py` (Hauptschleife), `read_live.py` (manuelles Live-Auslesen beider Boards, nuetzlich zum Kalibrieren), `control_state.py`/`set_control.py` (Fernsteuerung vom Handy, Challenge II Track C #195 - siehe `Challenge-II-Ice-Truck-Extension/Code/README.md`), `tests/`
